...
export async function generateMetadata({ params }) {
    ...
    return {
        title: project.title,
        description: project.description?.substring(0, 160) || `Découvrez notre réalisation : ${project.title}`,
        alternates: {
            canonical: `/project-details/${params.slug}`,
        },
        openGraph: {
            title: project.title,
            description: project.description?.substring(0, 160),
            images: project.imgSrc ? [{ url: project.imgSrc }] : [],
        },
    };
}
        ...
        <>
            ...
            <article className="project__details py__130">
                <div className="container">
                    <SlideUp>
                        <h2 className="title title__black text-center">{project.title}</h2>
                    </SlideUp>
                    <SlideUp>
                    ...
                    <div className="first__para">
                        ...
                        <h3 className="t__28">Description</h3>
                        ...
                        <div className="row pt__50">
                            {project.gallery.map((img, idx) => (
                                <div key={idx} className="col-md-4 mb-3">
                                    <img src={img} alt={`${project.title} - image ${idx + 1}`} className="w-100 rounded-3" />
                                </div>
                            ))}
                        </div>
                    )}
